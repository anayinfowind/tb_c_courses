...
/**
 * Helper functions for tb_c_courses block
 *
 * @package   block_tb_c_courses
 * @copyright  2020 Leeloo LXP (https://leeloolxp.com)
 * @author     Leeloo LXP <info@leeloolxp.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
defined('MOODLE_INTERNAL') || die;
define('BLOCKS_TB_C_COURSES_SHOWCATEGORIES_NONE', '0');
define('BLOCKS_TB_C_COURSES_SHOWCATEGORIES_ONLY_PARENT_NAME', '1');
define('BLOCKS_TB_C_COURSES_SHOWCATEGORIES_FULL_PATH', '2');
define('BLOCKS_TB_C_COURSES_IMAGEASBACKGROUND_FALSE', '0');
define('BLOCKS_TB_C_COURSES_SHOWGRADES_NO', '0');
define('BLOCKS_TB_C_COURSES_SHOWGRADES_YES', '1');
define('BLOCKS_TB_C_COURSES_STARTGRID_NO', '0');
define('BLOCKS_TB_C_COURSES_STARTGRID_YES', '1');
define('BLOCKS_TB_C_COURSES_DEFAULT_COURSES_ROW', '4');
define('BLOCKS_TB_C_COURSES_DEFAULT_COL_SIZE', '3');
define('BLOCKS_TB_C_COURSES_SHOWTEACHERS_NO', '0');
define('BLOCKS_TB_C_COURSES_SHOWTEACHERS_YES', '1');
 ...
 */
function block_tb_c_courses_get_overviews($courses) {
    $htmlarray = array();
...
/**
 * Sets user preference for maximum courses to be displayed in tb_c_courses block
 *
 * @param int $number maximum courses which should be visible
 */
function block_tb_c_courses_update_mynumber($number) {
    set_user_preference('tb_c_courses_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in tb_c_courses block
 *
 * @param array $sortorder list of course ids
 */
function block_tb_c_courses_update_myorder($sortorder) {
    ...
    set_user_preference('tb_c_courses_course_sortorder', $value);
}

/**
 * Gets user course sorting preference in tb_c_courses block
 *
 * @return array list of course ids
 */
function block_tb_c_courses_get_myorder() {
    if ($value = get_user_preferences('tb_c_courses_course_sortorder')) {
        return explode(',', $value);
    }
    // If preference was not found, look in the old location and convert if found.
    $order = array();
    if ($value = get_user_preferences('tb_c_courses_course_order')) {
        $order = unserialize_array($value);
        block_tb_c_courses_update_myorder($order);
        unset_user_preference('tb_c_courses_course_order');
    }
    return $order;
}
 ...
 */
function block_tb_c_courses_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_tb_c_courses', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_tb_c_courses', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_tb_c_courses', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in tb_c_courses block
 *
...
function block_tb_c_courses_get_max_user_courses($showallcourses = false) {
    ...
    if (!$output = $curl->post($url, $postdata, $options)) {
        $limit = get_user_preferences('tb_c_courses_number_of_courses', $limit);
    }
    ...
    $settingleeloolxp = $resposedata->data->completed_courses;

    $limit = $settingleeloolxp->completed_defaultmaxcourses;

    // If max course is not set then try get user preference.
    if (empty($settingleeloolxp->completed_forcedefaultmaxcourses)) {
        if ($showallcourses) {
            $limit = 0;
        } else {
            $limit = get_user_preferences('tb_c_courses_number_of_courses', $limit);
 ...
 */
function block_tb_c_courses_get_sorted_courses($showallcourses = false, $categoryid = 1) {
    global $USER;
    global $DB;

    $categorythispath = '/' . $categoryid . '/';

    $limit = block_tb_c_courses_get_max_user_courses($showallcourses);
    ...
    $order = block_tb_c_courses_get_myorder();
 ...
 * @return string
 */
function block_tb_c_courses_build_progress($course, $config) {
    global $CFG;
